...
(function($) {
   ...
   $.widget("ui.weekCalendar", {
      options: {        
         ...
         use24Hour : true,
         daysToShow : 7,
		 minBodyHeight: 1550,// regola la minima altezza delle calendario
         ...
         startParam : "start",
         endParam : "end",
         businessHours : {start: 6, end: 22, limitDisplay : true},// quando inizi a quando finisce il calendario ore vere + 4
         newEventText : "New Event",
         timeslotHeight: 16,// sto coso determiana l'altezza dell'evento vedi riga 55/56
         ...
         // Cassapanco: cose che ho aggiunto io
         displayEventHeader: false, // dice se visualizzare o meno l'header gegli eventi
         eventSeparation: 4, // quanti pixel vuoti lasciare (in basso) tra un evento ed il successivo
         // fine delle cose che ho aggiunto io
		...
		$('#caption').remove()
         ...
         shortMonths : ['Gen', 'Feb', 'Mar', 'Apr', 'Mag', 'Gun', 'Lug', 'Ago', 'Set', 'Ott', 'Nov', 'Dic'],
         longMonths : ['Gennaio', 'Febbraio', 'Marzo', 'Aprile', 'Maggio', 'Giugno', 'Luglio', 'Agosto', 'Settembre', 'Ottobre', 'Novembre', 'Dicembere'],
	 ...
	 //console.log("Y:"+self.options.date.getHours());
...
//        renderRow+= "</tr>";
      ...
      _renderEvent: function (calEvent, $weekDay) {
         ...
         // Cassapanco: aggiungo la possibilità di rimuovere l'header che indica il tempo 
         // di inizio e di fine di ogni evento nella speranza che sia piu' leggibile.
         // Ho aggiunto l'opzione: displayEventHeader
         eventHtml = "<div class=\"" + eventClass + " ui-corner-all\">" +
            (options.displayEventHeader ? "<div class=\"wc-time ui-corner-top\"></div>" : "") +
      ...
      _positionEvent : function($weekDay, $calEvent) {
         ...
         var pxHeight = pxPerMillis * eventMillis - options.eventSeparation;
       ...
       */
      _scrollToHour: function(h, animate) {
	 var hour =h-4;
         var self = this;
         var options = this.options;
