...
function retplis(){
	var e=[];
	$.ajax({url: 'repliche.json',async: false,dataType: 'json',success: function(d){$.each(d.events, function(i,item){e.push(jstoelem(item));});} });
return e;};


function inonda(){
	$('#nellaria').html("Musica NO STOP");
	$('#onair').css({"background":"transparent url(/image/layout/onair.png) no-repeat 0px -46px"});
	$('#offair h1').html(" ");
	dn=Date.parse('now');
	dnh=dn.getHours();
	dnm=dn.getMinutes();
	$.getJSON('repliche.json', function(d) {
	  	$.each(d.events, function(i,item){
				...
				if (item.stato!=0){
					if ((elem.id!=0)&&(dn.toString('ddd MMM d') == elem.start.toString('ddd MMM d'))&&(dnh>=h1)&&(dnh<=h2)&&((dnh==h2)?(dnm<=elem.end.getMinutes()):(1==1))){
						...
						$('#onair').css({"background":"transparent url(/image/layout/onair.png) no-repeat top left"});
